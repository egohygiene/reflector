...
-----
    ...
    python scripts/sync-version.py --version X.Y.Z  # override version instead of reading VERSION
...
#
# Each helper returns True when the file already matches the target version
# (no change required) and False when a change is needed.  In --check mode
# callers skip the write and report drift.
# ---------------------------------------------------------------------------
...
def _sync_publication_yaml(path: Path, version: str, check: bool) -> bool:
    ...
    # Preserve the rest of the file and update only the version line.
    # We use a targeted regex replacement to avoid reformatting the YAML.
    text = path.read_text(encoding="utf-8")
    updated = re.sub(
        r'^(version:\s+)["\']?\d+\.\d+\.\d+["\']?',
        f'\\g<1>"{version}"',
...
def _sync_release_please_manifest_json(path: Path, version: str, check: bool) -> bool:
    """Sync .release-please-manifest.json root package version."""
    data = load_json(path)
    if data is None:
        return False
    if str(data.get(".")) == version:
        return True
    if check:
        log(f"  DRIFT  {path.name}: '.' '{data.get('.')}' != '{version}'")
        return False
    data["."] = version
    write_json(path, data)
    log(f"  UPDATED {path.name}: '.' → '{version}'")
    return True
...
def main() -> int:
    ...
    surfaces = [
        ...
        (
            ...
            _sync_release_please_manifest_json,
        ),
    ]
