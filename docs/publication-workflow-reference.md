...
| `build-paper` | `validate` | Builds the paper PDF and verifies the staged arXiv source preflight |
...
|---|---|---|
| `build-paper` | `reflector-pdf` | Canonical `paper.pdf` |
| `build-arxiv` | `reflector-arxiv-bundles` | Staged source archives, validation report, and staged-source PDF |
...
**Scope routing:**
- `paper/**` changes: canonical build plus staged arXiv preflight; the incremental publication path emits arXiv archives (no GitHub Release)
